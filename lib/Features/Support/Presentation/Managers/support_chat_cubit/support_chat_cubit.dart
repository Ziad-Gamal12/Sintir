...
class SupportChatCubit extends Cubit<SupportChatState> {
  ...
  Future<void> sendMessage(
    ...
    final response =
        await supportChatRepo.sendMessage(message: message, ticketId: ticketId);
    response.fold(
        (failure) => emit(SendMessageFailure(
  ...
  Future<void> removeMessage(
    ...
    final response = await supportChatRepo.removeMessage(
        messageId: messageId, ticketId: ticketId);
    response.fold(
        (failure) => emit(RemoveMessageFailure(
              errMessage: failure.message,
            )),
  ...
  Future<void> updateMessageContent(
    ...
    final response = await supportChatRepo.updateMessageContent(
        ...
        messageId: messageId);
    response.fold(
        (failure) => emit(UpdateMessageContentFailure(
              errMessage: failure.message,
            )),
  ...
  Future<void> pickAndUplaodImage() async {
    ...
    response.fold(
        (failure) => emit(PickAndUploadMessageImageFailure(
              errMessage: failure.message,
            )),
        (url) => emit(PickAndUploadMessageImageSuccess(
              imageUrl: url,
            )));
  }
}
